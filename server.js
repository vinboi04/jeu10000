...
async function initDb() {
  ...
  `);
  // Table pour l'état de partie en direct (mode spectateur)
  await pool.query(`
...
// ═══════════════════════════════════════════
// PARTIE EN DIRECT (mode spectateur)
// ═══════════════════════════════════════════
...
app.get('/api/live-game/:key', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT master_name, data, updated_at FROM live_games WHERE master_key = $1',
      [req.params.key]
    );
    ...
    if (Date.now() - updatedAt > LIVE_GAME_TIMEOUT_MS) {
...
app.post('/api/live-game/:key', async (req, res) => {
  try {
    const { masterName, data } = req.body;
    await pool.query(
      `INSERT INTO live_games (master_key, master_name, data, updated_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (master_key) DO UPDATE SET master_name = $2, data = $3, updated_at = NOW()`,
      [req.params.key, masterName || req.params.key, JSON.stringify(data)]
    );
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
...
// DELETE — le maître arrête de partager sa partie (fin normale ou abandon)
...
// GET la liste des parties actives (pour "rejoindre une partie"), nettoie au passage les expirées
app.get('/api/live-games', async (req, res) => {
  try {
    await pool.query(
      'DELETE FROM live_games WHERE updated_at < NOW() - INTERVAL \'1 hour\''
    );
    const result = await pool.query(
      'SELECT master_key, master_name, data, updated_at FROM live_games ORDER BY updated_at DESC'
    );
    const games = result.rows.map(row => ({
      masterKey: row.master_key,
      masterName: row.master_name,
      playerCount: Array.isArray(row.data?.players) ? row.data.players.length : 0,
